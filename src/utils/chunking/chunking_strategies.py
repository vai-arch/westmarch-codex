...
class CHUNK_TYPES(str, Enum):
    RAW = "RAW"
    SEARCH = "SEARCH"
    DOCUMENT = "DOCUMENT"


def semantic_chunker_function(text: str, chunking_strategy_config: Dict) -> List[str]:
    """
    Semantic chunker with PROPER overlap implementation.

    Overlap strategy:
    - Chunks are built independently
    - After building chunk N, we calculate overlap
    - Chunk N+1 STARTS with overlap units from chunk N
    - We track boundaries carefully to avoid infinite loops
    """
    if not text.strip():
        return []

    # Extract config
    max_tokens = chunking_strategy_config.get("SEMANTIC_MAX_CHUNK_TOKENS", 480)
    min_tokens = chunking_strategy_config.get("SEMANTIC_MIN_CHUNK_TOKENS", 240)
    overlap_tokens = chunking_strategy_config.get("SEMANTIC_OVERLAP_TOKENS", 0)
    similarity_threshold = chunking_strategy_config.get("SEMANTIC_SIMILARITY_THRESHOLD", 0.80)
    base_unit = chunking_strategy_config.get("BASE_UNIT", "sentence")
    embedding_manager_config = chunking_strategy_config.get("EMBEDDING_MANAGER_CONFIG", {})

    # Get embedding manager
    global _embedding_manager
    if _embedding_manager is None:
        _embedding_manager = create_embedding_manager(embedding_manager_config)
    ...
    if base_unit == "paragraph":
        for para in paragraphs:
            para_tokens = _embedding_manager.token_count(para)
            if para_tokens <= max_tokens:
                processable_units.append(para)
            else:
                doc = nlp(para)
                for sent in doc.sents:
                    sent_text = sent.text.strip()
                    sent_tokens = _embedding_manager.token_count(sent_text)

                    # Force split if sentence itself is too large
                    if sent_tokens > max_tokens:
                        words = sent_text.split()
                        temp = []
                        temp_tokens = 0
                        for word in words:
                            word_tokens = _embedding_manager.token_count(word)
                            if temp_tokens + word_tokens > max_tokens:
                                if temp:
                                    processable_units.append(" ".join(temp))
                                temp = [word]
                        ...
                        processable_units.append(sent_text)

    elif base_unit == "sentence":
        for para in paragraphs:
            doc = nlp(para)
            for sent in doc.sents:
                sent_text = sent.text.strip()
    ...
    # Embed all units
    try:
        embed_result = _embedding_manager.embed_chunks(processable_units, show_progress=False)
        ...
        if embeddings.ndim != 2 or embeddings.shape[0] != len(processable_units):
            raise ValueError("Embedding shape mismatch")

    except Exception as e:
        print(f"⚠️ Embedding failed: {e}. Falling back to size-based chunking.")
        return _simple_size_chunking(text, processable_units, max_tokens, _embedding_manager)

    # CHUNKING WITH PROPER OVERLAP
    chunks = []
    i = 0  # Current position in processable_units

    while i < len(processable_units):
        chunk_units = []
        chunk_start_idx = i  # Remember where THIS chunk started

        # Build one chunk
        while i < len(processable_units):
            temp_chunks = " ".join(chunk_units)
            temp_chunks += " " + processable_units[i]
            total_tokens_so_far = _embedding_manager.token_count(temp_chunks)

            # Would exceed max?
            if total_tokens_so_far > max_tokens:
                break

            # Add the unit
            chunk_units.append(processable_units[i])
            i += 1

            # Check semantic boundary AFTER adding
            # Only check if:
            # 1. We've met minimum size
            # 2. We have at least 2 units in chunk
            # 3. There are more units to process
            if total_tokens_so_far >= min_tokens and len(chunk_units) >= 2 and i < len(processable_units):
                # Compare last unit added to chunk vs next unit in sequence
                last_idx = i - 1  # Last unit we just added
                next_idx = i  # Next unit we're considering

                similarity = 1 - cosine(embeddings[last_idx], embeddings[next_idx])
                if similarity < similarity_threshold:
                    break  # Semantic boundary found

        # Safety: ensure we made progress
        if not chunk_units:
            chunk_units = [processable_units[chunk_start_idx]]
            i = chunk_start_idx + 1
        ...
        # OVERLAP LOGIC: Move i backwards for next chunk to start with overlap
        if overlap_tokens > 0 and i < len(processable_units):
            # Calculate how many units from END of current chunk = overlap_tokens
            overlap_token_count = 0
            overlap_unit_count = 0

            for unit_idx in range(len(chunk_units) - 1, -1, -1):
                unit = chunk_units[unit_idx]
                unit_tokens = _embedding_manager.token_count(unit)

                # Would this unit make overlap too large?
                if overlap_token_count + unit_tokens > overlap_tokens * 1.5:
                    break

                overlap_token_count += unit_tokens
                overlap_unit_count += 1

                # Have we met the overlap target?
                if overlap_token_count >= overlap_tokens:
                    break

            # Rewind i by the number of units that should overlap
            # BUT: ensure we move forward by at least 1 unit
            if overlap_unit_count > 0 and overlap_unit_count < len(chunk_units):
                # Next chunk starts at: current position minus overlap
                i = max(chunk_start_idx + 1, i - overlap_unit_count)

    return chunks


def _simple_size_chunking(text: str, units: List[str], max_tokens: int, embedding_manager) -> List[str]:
    """
    Fallback: simple size-based chunking when embeddings fail.
    """
    chunks = []
    current_chunk = []
    current_tokens = 0
    ...
    for unit in units:
        unit_tokens = embedding_manager.token_count(unit)
...
if __name__ == "__main__":
    paragraphs = [
        ...
        '"A little beer would suit me better, if it is all the same to you, my good sir," said Balin with the white beard. "But I don\'t mind some cake-seedcake, if you have any."',
        ...
        'What\'s that? Tea! No thank you! A little red wine, I think, for me." "And for me," said Thorin. "And raspberry jam and apple-tart," said Bifur. "And mincepies and cheese," said Bofur. "And pork-pie and salad," said Bombur. "And more cakes-and ale-and coffee, if you don\'t mind," called the other dwarves through the door.',
        ...
        "This was Thorin's style. He was an important dwarf. If he had been allowed, he would probably have gone on like this until he was out of breath, without telling any one there 'anything that was not known already. But he was rudely interrupted. Poor Bilbo couldn't bear it any longer. At may never return he began to feel a shriek coming up inside, and very soon it burst out like the whistle of an engine coming out of a tunnel. All the dwarves sprang Bp knocking over the table. Gandalf struck a blue light on the end of his magic staff, and in its firework glare the poor little hobbit could be seen kneeling on the hearth-rug, shaking like a jelly that was melting. Then he fell flat on the floor, and kept on calling out \"struck by lightning, struck by lightning!\" over and over again; and that was all they could get out of him for a long time. So they took him and laid him out of the way on the drawingroom sofa with a drink at his elbow, and they went back to their dark business.",
        ...
        'In fact, if it bad not been for the sign on the door, I should have been sure we had come to the wrong house. As soon as I clapped eyes on the little fellow bobbing and puffing on the mat, I had my doubts. He looks more like a grocerthan a burglar!" Then Mr. Baggins turned the handle and went in. The Took side had won. He suddenly felt he would go without bed and breakfast to be thought fierce. As for little fellow bobbing on the mat it almost made him really fierce. Many a time afterwards the Baggins part regretted what he did now, and he said to himself: "Bilbo, you were a fool; you walked right in and put your foot in it."',
        ...
        '"That would be no good," said the wizard, "not without a mighty Warrior, even a Hero. I tried to find one; but warriors are busy fighting one another in distant lands, and in this neighbourhood heroes are scarce, or simply lot to be found. Swords in these parts are mostly blunt, and axes are used for trees, and shields as cradles or dish-covers; and dragons are comfortably faroff (and therefore legendary). That is why I settled on burglary-especially when I remembered the existence of a Side-door. And here is our little Bilbo Baggins, the burglar, the chosen and selected burglar. So now let\'s get on and make some plans."',
    ...
    CHUNKING_STRATEGY = {
        ...
        "SEMANTIC_MAX_CHUNK_TOKENS": 500,
        # High floor ensures substantive content
        "SEMANTIC_MIN_CHUNK_TOKENS": 250,  # MAX / 2
        # Goldilocks overlap: enough for boundaries, not wasteful
        "SEMANTIC_OVERLAP_TOKENS": 50,  # MAX * 0.1
        # YOUR tested value - don't change what works!
        "SEMANTIC_SIMILARITY_THRESHOLD": 0.75,
        # Strong minimum prevents fragments
        "MIN_BOOKS_CHUNKS_SIZE_TOKENS": 60,
    }

    text = "\n\n".join(paragraphs)

    chunks = semantic_chunker_function(text=text, chunking_strategy_config=CHUNKING_STRATEGY)
    ...
    for idx, chunk in enumerate(chunks):
        chunk_size_tokens = embedding_manager.token_count(chunk)
        token_counts.append(chunk_size_tokens)

        print(f"Chunk {idx + 1} | Tokens: {chunk_size_tokens}")
        print(chunk)
        print("-" * 80)

    # ---- Summary stats (ignore last chunk) ----
    stats_tokens = token_counts[:-1] if len(token_counts) > 1 else token_counts

    if stats_tokens:
        min_tokens = min(stats_tokens)
        max_tokens = max(stats_tokens)
        avg_tokens = sum(stats_tokens) / len(stats_tokens)

        print("\n📊 CHUNK TOKEN STATISTICS (excluding last chunk)")
        print(f"Chunks used : {len(stats_tokens)}")
        print(f"Min tokens  : {min_tokens}")
        print(f"Max tokens  : {max_tokens}")
        print(f"Avg tokens  : {avg_tokens:.1f}")

        # Optional: flag dangerous chunks
        print("\n⚠️  Potential problems:")
        for i, t in enumerate(stats_tokens):
            if t < 100:
                print(f"  Chunk {i + 1} is VERY SMALL ({t} tokens)")
            elif t > 1000:
                print(f"  Chunk {i + 1} is VERY LARGE ({t} tokens)")
